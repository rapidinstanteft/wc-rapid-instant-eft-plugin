...
/**
 * WC_RapidPayment_API class.
 *
 * Communicates with Rapid Payment API.
 */
class WC_RapidPayment_API {
	...
	public static function get_username() {
		if ( ! self::$username ) {
			$options = get_option( 'woocommerce_rapidpayment_settings' );
	...
	public static function request( $request, $api = 'user/login', $method = 'POST' ) {
		...
		WC_RapidPayment::log( "{$api} request to ".$url . print_r( $request, true ) );
		...
		$response = wp_remote_post(
			...
			array(
				...
				'body'       => apply_filters( 'woocommerce_rapidpayment_request_body', $request, $api ),
				'timeout'    => 70,
        ...
        if (is_wp_error($response)) {
            WC_RapidPayment::log( "WP Error: ".$response->get_error_message());
            return $response;
        }

        WC_RapidPayment::log( "Response: ".json_encode($response));

        $parsed_response = json_decode( $response['body'] );


        if(empty( $response['body'] ) ) {
            WC_RapidPayment::log( "Error Response: " . print_r( $response, true ) );
            return new WP_Error( 'rapidpayment_error', __( 'There was a problem connecting to the payment gateway.'.$parsed_response->name, 'wc-gateway-rapidpayment' ) );
        }
	...
	public static function get_token_data() {
        ...
        if ( is_wp_error( $response ) ) {
            WC_RapidPayment::log( 'Rapid Payment Token API Error: '.$response->get_error_message() );
    ...
    public static function get_transaction_data($transaction_id) {
        ...
        if ( is_wp_error( $response ) ) {
            WC_RapidPayment::log( 'Rapid Payment Transaction API Error: '.$response->get_error_message() );
        }
        return $response;
    }
    ...
    public static function get_payment_key_data($data = []) {
        ...
        if ( is_wp_error( $response ) ) {
            WC_RapidPayment::log( 'Rapid Payment Payment Key API Error: '.$response->get_error_message() );
